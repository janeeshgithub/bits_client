...
export default function Recommendations() {
  ...
  const [savedProjects, setSavedProjects] = useState<Map<string, boolean>>(new Map());
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="max-w-2xl mx-auto mb-12">
          <div className="relative">
            ...
            <button
              ...
              disabled={isLoading}
            >
              {isLoading ? "Generating Roadmap..." : "Generate Learning Roadmap"}
            </button>
          </div>
        </div>
        ...
        {!isLoading && results.length > 0 && (
          <div className="space-y-8">
            {results.map((project) => (
              <div key={project.title} className="bg-gray-800 rounded-lg p-6 border border-gray-700 hover:border-blue-500 transition-colors">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-gray-300">{project.difficultyLevel}</span>
                  </div>
                ...
                <div className="mt-4">
                  <h4 className="text-sm font-semibold text-blue-400 mb-2">Roadmap:</h4>
                  <ul className="list-disc list-inside text-gray-400 text-sm">
                    {project.roadmap.map((step, index) => (
                      <li key={index}>{step.step} - <span className="text-blue-400">{step.difficulty}</span></li>
                    ))}
                  </ul>
                </div>
              </div>
